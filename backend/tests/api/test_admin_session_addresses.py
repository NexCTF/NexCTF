...
from nexctf.model import Event, User, UserSession

from ..base import ListGuardMixin, get_data, make_team, make_user
...
async def _overview(client, query: str = "") -> dict:
    return await get_data(client, f"{ADDRESSES}{query}")
...
class TestListSessionAddresses(ListGuardMixin):
    ...
    async def test_same_team_sharing_is_flagged(self, admin_client, db_session):
        c, _ = admin_client
        team = await make_team(db_session, "MyTeam")
        alice = await make_user(db_session, "alice", team)
        bob = await make_user(db_session, "bob", team)
    ...
    async def test_cross_team_sharing_counts_both_teams(self, admin_client, db_session):
        ...
        bob = await make_user(db_session, "bob", await make_team(db_session, "Blue"))
    ...
    async def test_teamless_accounts_are_listed_and_never_same_team(
        ...
        team = await make_team(db_session, "MyTeam")
        alice = await make_user(db_session, "alice", team)
        nomad = await make_user(db_session, "nomad")
    ...
    async def test_most_shared_address_comes_first(self, admin_client, db_session):
        c, _ = admin_client
        users = [await make_user(db_session, f"u{n}") for n in range(3)]
        for user in users:
