...
- `dev:frontend:check` re-runs the parser with `--fail-on-update`, so unsynced keys fail CI
- Config lives in `frontend/i18next-parser.config.mjs`. `keepRemoved: true` is required: 9 call sites build the key at runtime and the parser cannot see them
